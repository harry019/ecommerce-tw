...
fetch("https://dummyjson.com/products").then((e)=>e.json()).then((e)=>
{
    ...
    allProduct=e.products
}).catch((err)=>{
    console.log(err.message)
    alert(err.message)
})
//logic to display data on ui
function displayData(product){
  ...
  <div class="mt-4">
    ...
    <div class="flex items-end justify-between">
      ...
      <span class="text-blue-600 text-xl font-semibold">${val.price}-(${val.price}*${val.discountPercentage})/${100}</span>
      <span class="text-gray-400 text-sm line-through">${val.price}</span>
    </div>
     <button
    ...
    allproduct.innerHTML=input
  }
  function getId(id){
  let single_product=allProduct.find((val)=>{
    return val.id==id
  })
